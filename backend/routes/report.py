"""
GET /api/report/latest           â€” most recent scan from MongoDB
GET /api/report/scans            â€” all past scans (history)
GET /api/report/feature/{id}     â€” single feature with competitive intel
GET /api/report/download         â€” download latest as markdown
POST /api/report/post-to-gitlab  â€” post graveyard summary as a GitLab issue (native CI integration)
"""
...
async def list_scans():
    """Return all past scans â€” chronological history."""
    if not settings.MONGODB_URI:
...
async def get_all_features(limit: int = 200):
    """
    Return all features from ALL scans â€” used by Timeline Forensics to show
...
async def post_report_to_gitlab(req: PostToGitLabRequest):
    ...
    lines = [
        f"## ðŸª¦ Feature Graveyard Report â€” {scan_date}",
        "",
        "> Generated by **NECRO** Â· Google Cloud Agent Builder Â· Gemini 3 Flash Â· GitLab MCP",
        "",
        ...
        f"**Found:** {len(features)} dead features â€” "
        f"{len(revive_now)} ready to revive Â· {len(investigate)} investigate Â· {len(keep_buried)} keep buried",
        "",
    ]
    ...
    if revive_now:
        lines += [
            "---",
            "",
            f"### ðŸ”¥ Ready to Revive ({len(revive_now)})",
            "",
        ...
        for f in revive_now:
            ...
            what_changed = vi.get("what_changed", "â€”")
    ...
    if investigate:
        lines += [
            "---",
            "",
            f"### ðŸ” Investigate Further ({len(investigate)})",
            "",
        ]
        for f in investigate:
            ...
            vi = f.get("viability", {})
            lines.append(
                f"- **{f.get('name', '?')}** â€” {(dr.get('primary_reason') or '?')[:80]}  "
    ...
    lines += [
        ...
        "(Vertex AI Gemini 3 Flash) that stress-tests the primary finding from a "
    ...
    issue_body = "\n".join(lines)
    issue_title = f"ðŸª¦ NECRO Graveyard Report â€” {len(revive_now)} features ready to revive ({scan_date})"
    ...
    if not issue:
        raise HTTPException(status_code=503, detail="GitLab issue creation failed â€” no response from API. Check GITLAB_TOKEN.")
    ...
    if issue.get("_error"):
        ...
        if gl_status == 403:
            raise HTTPException(
                status_code=403,
                detail=f"Permission denied â€” your token needs Developer+ access to '{req.project_path}'. Change the project path to one you own.",
            )
        elif gl_status == 404:
            raise HTTPException(
                status_code=404,
                detail=f"Project '{req.project_path}' not found â€” paste a valid GitLab project path (e.g. your-namespace/your-repo).",
            )
        elif gl_status == 401:
            raise HTTPException(
                status_code=401,
                detail="GITLAB_TOKEN is invalid or expired â€” check your .env.",
            )
        else:
            raise HTTPException(
                status_code=502,
...
async def notify_slack(req: NotifySlackRequest):
    ...
    if not ok:
        raise HTTPException(status_code=502, detail="Slack delivery failed â€” check your webhook URL or bot token.")
